...
python -m scripts.check_env_versions
```

If your local copy does not contain `scripts/check_env_versions.py`, you’re on an older snapshot—run `git pull` (or re-download) to sync with this repo. As a fallback, you can also check versions without the helper:

```bash
python - <<'PY'
from importlib import metadata
for pkg in ["sqlalchemy", "typing_extensions"]:
    try:
...
The FastAPI UI now checks the installed SQLAlchemy version at startup and will
raise a clear error if it is too old, pointing you to the reinstall command
above so you avoid the obscure `TypingOnly` assertion.
